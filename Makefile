...
test-relation-integration: check-services
...
test-integration: start-relation-infrastructure test-relation-integration stop-relation-infrastructure
...
start-dev-full: setup-monitoring start-monitoring start-relation-infrastructure
	...
	@echo "  💕 Relation Service: http://localhost:8083"
...
stop-dev-full: stop-monitoring stop-relation-infrastructure
	@echo "🛑 Полная dev среда остановлена"

# Очистка всей dev среды
clean-dev-full: clean-monitoring clean-relation-infrastructure
	@echo "🧹 Полная dev среда очищена"

# Запуск только с Prometheus stack (без ELK)
start-dev-light: setup-monitoring start-prometheus-stack start-relation-infrastructure
